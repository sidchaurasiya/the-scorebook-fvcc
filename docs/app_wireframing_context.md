...
- Player Profile production now includes `Recent Form ⚡` below Career Overview. It is scorecard-based, latest-first, desktop latest 10 chips, mobile latest 5 chips, with batting not-out stars and balls only where reliable; the visible helper copy has been removed for a cleaner card.
- Season Overview production now includes `Season by Round 🗓️` above Season Standouts. It is scorecard-safe, uses an in-card grade/team segmented control for multi-team seasons, excludes the `All` option, keeps the URL unchanged, shows latest five rows before internal scroll, and uses full cricket result wording.
...
- Season by Round performer logic: Best Batter is the top FVCC scorecard batting score for the match; Best Bowler sorts FVCC bowling figures by wickets descending and runs ascending. Missing scorecard/match rows render a compact empty state.
