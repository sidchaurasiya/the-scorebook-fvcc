...
- Player Profile production now includes `Recent Form ⚡` below Career Overview. It is scorecard-based, latest-first, desktop latest 10 chips, mobile latest 5 chips, with batting not-out stars and balls only where reliable.
- Season Overview production now includes `Season by Round 🗓️` above Season Standouts. It is scorecard-safe, uses an in-card grade/team segmented control for multi-team seasons, keeps the URL unchanged, shows latest five rows before internal scroll, and uses full cricket result wording.
