...
- Folder-tab implementation note: the approved folder-tab wireframe has been applied to Season by Round grade/team selection, the Milestone main view selector, and Player Profile Career Breakdown dimensions. These controls use shared session-state widget behaviour, stay attached to the card/visual, and do not update URL query params.
- Player Profile Career Breakdown now has a `Captain` folder tab. It renders Captain rows from deploy-safe Player Profile performance summaries only when reliable captain context exists; otherwise the user sees a clean empty state. Batting / Bowling / Fielding stays as the existing pill-style discipline toggle, left-aligned inside the card.
