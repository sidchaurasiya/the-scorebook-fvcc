...
- Current Player Profile mobile polish: Career Overview hides Fielding on mobile for non-keepers, Career Highlights uses a compact two-card mobile grid, Career Breakdown controls are embedded in the section card, Bowling Phase stacks `Best phase` under the phase name on mobile with a subtle BBB coverage note, and Dismissal Fingerprint insight sentences now call out dismissal types only when they are 10+ points above club average.
