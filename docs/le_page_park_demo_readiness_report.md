...
- Those included in deploy-safe Fastest 100s: 3
- Those excluded: 5
- Why they were excluded:
  - 5 innings are scorecard-only and do not have verified BBB coverage, so the balls-to-100 timing cannot be claimed.
  - Steve McConchie’s 175 was included via an explicit Le Page demo exception, because the ball-by-ball progression is still readable and reaches the scorecard total even though one cumulative source line in the raw feed is inconsistent.
- Conclusion: the current Fastest 100s output is intentionally conservative, with one approved demo exception for Steve McConchie.
