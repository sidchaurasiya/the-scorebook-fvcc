...
- Player Profile now includes `Recent Form ⚡` directly below Career Overview. It uses existing scorecard innings/spells from the local match-centre processed export, orders latest first, shows desktop latest 10 chips, and hides to latest 5 chips on mobile.
- Season Overview now includes `Season by Round 🗓️` above Season Standouts. It uses existing match-centre scorecard rows, groups multi-team seasons by grade/team, provides an in-card session-state grade/team toggle, shows latest five results before internal scroll, and uses complete result wording such as `won by 42 runs` / `lost by 5 wickets`.
