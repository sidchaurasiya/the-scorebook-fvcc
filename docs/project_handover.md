...
- Player Profile now includes `Recent Form ⚡` directly below Career Overview. It uses existing scorecard innings/spells from the local match-centre processed export, orders latest first, shows desktop latest 10 chips, hides to latest 5 chips on mobile, and no longer displays the `Latest first · 10 shown` helper copy.
- Season Overview now includes `Season by Round 🗓️` above Season Standouts. It uses existing match-centre scorecard rows, shows one selected grade/team at a time, provides an in-card session-state grade/team toggle without an `All` option, shows latest five results before internal scroll, and uses complete result wording such as `won by 42 runs` / `lost by 5 wickets`.
...
- Season by Round Best Batter is the highest FVCC scorecard batting score in the match, including `*` for not-outs; Best Bowler sorts FVCC scorecard bowling by wickets descending then runs ascending. Missing match-level data should render the compact empty state, not fake rows.
