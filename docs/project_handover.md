...
- Current data refresh state: use `scripts/refresh_data.py --club fvcc --with-current-match-centre`, then `scripts/refresh_club_outputs.py --club fvcc`. Aggregate totals must refresh before deploy-safe summaries so Season Standouts and Detailed Stats do not lag behind Season by Round. Winter 2026 now includes R1-R4, and Season by Round should show R4 first.
