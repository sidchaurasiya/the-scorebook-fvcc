...
def calculate_carry_jobs(batting: pd.DataFrame, limit: int = 8) -> list[dict[str, Any]]:
    ...
    rows["next_highest"] = next_highest_scores(rows)
...
def next_highest_scores(batting: pd.DataFrame) -> pd.Series:
    """Return each batter's next-highest teammate score without row-by-row scans."""
    if batting.empty:
        return pd.Series(dtype="float64")
    group_columns = ["match_id", "innings_id"]
    missing = [column for column in group_columns if column not in batting.columns]
    if missing:
        return pd.Series(0.0, index=batting.index)
    rows = batting.copy()
    rows["_group_key"] = list(zip(rows["match_id"].astype(str), rows["innings_id"].astype(str)))
    rows["_runs"] = num(rows.get("runs_scored"))
    ranked = rows.sort_values(["_group_key", "_runs"], ascending=[True, False])
    group_stats = ranked.groupby("_group_key")["_runs"].agg(
        top_score="max",
        top_count=lambda values: int((values == values.max()).sum()),
        second_score=lambda values: float(values.nlargest(2).iloc[-1]) if len(values) > 1 else 0.0,
    )
    rows = rows.join(group_stats, on="_group_key")
    next_highest = rows["top_score"].where((rows["_runs"] < rows["top_score"]) | (rows["top_count"] > 1), rows["second_score"])
    return next_highest.fillna(0.0).reindex(batting.index)
