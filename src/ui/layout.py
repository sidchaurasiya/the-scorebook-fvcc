...
def active_runs_chart_colour() -> str:
    return active_link_colour() if get_active_club_id() == "fvcc" else active_chart_primary_colour()
...
def render_page() -> None:
    ...
    if selected_page == "hall-of-fame":
        ...
        record_club_load_profile("season_overview_build", (time.perf_counter() - route_started_at) * 1000)
        ...
        record_club_load_profile("milestones_build", (time.perf_counter() - route_started_at) * 1000)
    elif selected_page == PLAYER_PROFILE_QUERY_PAGE:
        route_started_at = time.perf_counter()
        render_player_profile_page()
        record_club_load_profile("player_profile_build", (time.perf_counter() - route_started_at) * 1000)
...
def render_data_source_panel(
    ...
    try:
        ...
        if using_local_backup:
            seasons = load_local_playcricket_seasons(get_active_club_id(), local_version)
        else:
...
def filter_empty_grdcc_season_teams(season: dict, teams: list[dict], local_version: float) -> list[dict]:
    ...
    for category in ["batting", "bowling", "fielding"]:
        frame = load_local_category_frame(
            get_active_club_id(), category, season_id, None, local_version, player_aliases_mtime()
        )
        if not frame.empty and "team_id" in frame:
            meaningful_team_ids.update(frame["team_id"].dropna().astype(str).str.strip())

    round_rows = load_season_overview_detail_sources(
        get_active_club_id(),
        season_overview_detail_source_signature(),
    ).get("season_by_round", pd.DataFrame())
...
def load_local_category_frame(
    ...
    _ = (club_id, local_version, identity_version)
...
def load_local_single_team_frames(
    ...
    batting = load_local_category_frame(club_id, "batting", season_id, team["id"], local_version, identity_version)
    bowling = load_local_category_frame(club_id, "bowling", season_id, team["id"], local_version, identity_version)
    fielding = load_local_category_frame(club_id, "fielding", season_id, team["id"], local_version, identity_version)
    return {
...
def load_local_all_team_frames(
    ...
    for category in ["batting", "bowling", "fielding"]:
        frame = load_local_category_frame(
            get_active_club_id(), category, season_id, None, local_version, identity_version
        )
        if not frame.empty and "team_id" in frame:
...
def add_season_overview_detail_metrics(
    ...
    sources = load_season_overview_detail_sources(
        get_active_club_id(),
        season_overview_detail_source_signature(),
    )
    team_ids = {team_id for team in teams for team_id in team_scope_ids(team)}
...
def render_season_by_round(dashboard_data: dict[str, object]) -> None:
    ...
    with st.container(key="season_by_round_card"):
        ...
        components.html(season_round_scroll_script_html(), height=0, scrolling=False)
...
def season_round_cards_html(rows: list[dict[str, object]], show_grade_column: bool = False) -> str:
    ...
    return (
        '<div class="season-round-scroll-shell" data-season-round-scroll-shell="true">'
...
def season_round_scroll_script_html() -> str:
    return """
    <script>
    (() => {
      const doc = window.parent && window.parent.document ? window.parent.document : document;
      doc.querySelectorAll('[data-season-round-scroll-shell="true"]').forEach((shell) => {
        if (shell.dataset.wheelBound === 'true') return;
        shell.dataset.wheelBound = 'true';
        const strip = shell.querySelector('.season-round-panel-strip');
        if (!strip) return;
        shell.addEventListener('wheel', (event) => {
          const maxScroll = strip.scrollWidth - strip.clientWidth;
          if (maxScroll <= 0 || Math.abs(event.deltaX) > Math.abs(event.deltaY)) return;
          const before = strip.scrollLeft;
          strip.scrollLeft = Math.max(0, Math.min(maxScroll, strip.scrollLeft + event.deltaY));
...
def season_round_premiership_match_ids() -> set[str]:
    wins, _players = load_premiership_records(get_active_club_id(), premiership_records_signature())
...
def load_player_dna_cached(
    ...
    _ = (local_version, identity_version, signature)
...
def render_hall_of_fame_page() -> None:
    ...
    if team_group_slug:
        ...
        single_fvcc_group = active_club_is_fvcc() and len(hall_of_fame_team_group_options(hall_of_fame_data)) <= 1
        if not single_fvcc_group:
            hall_of_fame_data = filter_hall_of_fame_data_by_team_group(hall_of_fame_data, team_group_slug)
        log_hof_timing(
            "Hall of Fame team-group aggregation",
            filter_started_at,
            cache_hit=single_fvcc_group,
            notes="Skipped equivalent FVCC single-group rebuild." if single_fvcc_group else "Applied selected team-group filter.",
        )
    section_started_at = time.perf_counter()
    render_premiership_records(team_group_slug)
    log_hof_timing("render premiership records", section_started_at)
    section_started_at = time.perf_counter()
    prepared_active_players = hall_of_fame_data.get("active_players")
    render_hall_of_fame_leaders(
        hall_of_fame_data["all_time"],
        prepared_active_players if isinstance(prepared_active_players, set) else active_hof_players(hall_of_fame_data),
    )
    ...
    if HALL_OF_FAME_PARTNERSHIP_RECORDS_PATH.exists():
        if get_active_club_id() in {"fvcc", "georges-river-district"}:
            render_verified_partnership_records(team_group_slug)
            log_hof_timing("render verified partnership records", section_started_at)
        else:
...
def filter_hall_of_fame_data_by_team_group(data: dict[str, object], team_group_slug: str) -> dict[str, object]:
    ...
    all_time = add_club_not_outs_for_display(all_time, batting, club_id)
    detailed_tables = {
...
def render_hall_of_fame_v2_premiership_wall() -> None:
    wins, players = load_premiership_records(get_active_club_id(), premiership_records_signature())
    st.markdown(
        (
...
def get_hall_of_fame_data(
    ...
    all_time = add_club_not_outs_for_display(all_time, historical_data["batting"], active_club_id)
    ...
    if club_id == "fvcc":
        prepared["active_players"] = active_hof_players(prepared)
    return prepared
...
def build_all_time_player_table(
    ...
    return coalesce_grdcc_exact_name_nonoverlap_all_time(all_time)
...
def coalesce_grdcc_exact_name_nonoverlap_all_time(all_time: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, group in all_time.groupby(all_time["Player"].map(player_name_match_key), sort=False, dropna=False):
        ...
        component_seasons: list[set[int]] = []
        for index, row in group.iterrows():
            seasons = all_time_row_season_range(row)
            placed = False
            for pos, existing in enumerate(component_seasons):
                if seasons and existing and seasons.isdisjoint(existing):
                    components[pos].append(index)
                    existing.update(seasons)
                    placed = True
                    break
            if not placed:
                components.append([index])
                component_seasons.append(set(seasons))
        for component in components:
            subset = group.loc[component]
            if len(subset) == 1:
                rows.append(subset.iloc[0])
            else:
                rows.append(aggregate_grdcc_exact_name_component(subset))
    if not rows:
        return all_time
    return pd.DataFrame(rows, columns=all_time.columns).reset_index(drop=True)


def all_time_row_season_range(row: pd.Series) -> set[int]:
    start = season_sort_value(row.get("Debut Season"))
    end = season_sort_value(row.get("Latest Season"))
...
def aggregate_grdcc_exact_name_component(component: pd.DataFrame) -> pd.Series:
    output = component.iloc[0].copy()
    player = safe_record_text(output.get("Player"), "Unknown player")
    output["canonical_player_id"] = f"grdcc_exact_name_{make_player_slug(player)}"
    output["player_key"] = output["canonical_player_id"]
    output["Player"] = player
    if "Teams/Grades" in component:
        output["Teams/Grades"] = join_unique_csv(component["Teams/Grades"])
    seasons = set()
    for _, row in component.iterrows():
        seasons.update(all_time_row_season_range(row))
    if seasons:
        output["Seasons Played"] = len(seasons)
        output["Debut Season"] = season_label_from_sort(min(seasons))
        output["Latest Season"] = season_label_from_sort(max(seasons))
    elif "Seasons Played" in component:
        output["Seasons Played"] = pd.to_numeric(component["Seasons Played"], errors="coerce").fillna(0).sum()
    ...
    for column in [
        ...
        if column in component:
            output[column] = pd.to_numeric(component[column], errors="coerce").fillna(0).sum()
    if "HS" in component:
        output["HS"] = best_high_score_label(component["HS"])
    if "BBI" in component:
        output["BBI"] = best_bowling_figures_label(component["BBI"])

    runs = pd.to_numeric(pd.Series([output.get("Runs")]), errors="coerce").iloc[0]
    innings = pd.to_numeric(pd.Series([output.get("Innings")]), errors="coerce").iloc[0]
    wickets = pd.to_numeric(pd.Series([output.get("Wickets")]), errors="coerce").iloc[0]
    balls = pd.to_numeric(pd.Series([output.get("Balls Bowled")]), errors="coerce").iloc[0]
    bowling_runs = pd.to_numeric(component.get("bowlingRuns", pd.Series(dtype="float64")), errors="coerce").fillna(0).sum()
    if pd.notna(runs) and pd.notna(innings) and innings > 0 and "Bat Avg" in output:
        output["Bat Avg"] = runs / innings
    if pd.notna(wickets) and wickets > 0 and bowling_runs > 0 and "Bowl Avg" in output:
        output["Bowl Avg"] = bowling_runs / wickets
    if pd.notna(wickets) and wickets > 0 and pd.notna(balls) and balls > 0 and "Bowl SR" in output:
        output["Bowl SR"] = balls / wickets
    if "Matches Source" in output:
        output["Matches Source"] = "historical_exact_name_nonoverlap_merge"
    return output


def season_label_from_sort(value: int) -> str:
    if value <= 0:
        return ""
    return f"Summer {value}/{str(value + 1)[-2:]}"


def best_high_score_label(values: pd.Series) -> str:
    best_label = ""
    best_score = -1
    best_not_out = False
    for value in values.dropna().astype(str):
        score = parse_high_score_value(value)
        if score[0] > best_score or (score[0] == best_score and score[1] and not best_not_out):
            best_score, best_not_out, best_label = score[0], score[1], value
    return best_label


def parse_high_score_value(value: str) -> tuple[int, bool]:
    match = re.search(r"\d+", value)
    return (int(match.group(0)) if match else -1, "*" in value)


def best_bowling_figures_label(values: pd.Series) -> str:
    best_label = ""
    best_wickets = -1
    best_runs = 10**9
    for value in values.dropna().astype(str):
        ...
        if not match:
            continue
        wickets, runs = int(match.group(1)), int(match.group(2))
        if wickets > best_wickets or (wickets == best_wickets and runs < best_runs):
            best_wickets, best_runs, best_label = wickets, runs, value
    return best_label
...
def premiership_records_signature() -> tuple[tuple[str, float], ...]:
    ...
    report_path = annual_report_premiership_path()
    if get_active_club_id() == "georges-river-district" and report_path.exists():
        signature.append((str(report_path), report_path.stat().st_mtime))
...
def render_verified_partnership_records(team_group_slug: str | None = None) -> None:
    render_section_heading("Verified Partnership Records 🤝")
    st.caption(
        "Highest partnerships by wicket identified from available verified ball-by-ball records. "
        "Private player identities are protected."
    )
    records = load_verified_partnership_records(
        get_active_club_id(),
        str(HALL_OF_FAME_PARTNERSHIP_RECORDS_PATH),
        HALL_OF_FAME_PARTNERSHIP_RECORDS_PATH.stat().st_mtime
        if HALL_OF_FAME_PARTNERSHIP_RECORDS_PATH.exists()
        else None,
    )
    if team_group_slug in {"men", "women"} and not records.empty:
        records = filter_hof_team_group_frame(records, team_group_slug)
    render_partnership_record_panels(
        records,
        empty_title="Verified Partnership Records",
        empty_message="No verified partnership records are available for this team group.",
    )


@st.cache_data(show_spinner=False, persist="disk")
def load_verified_partnership_records(
    club_id: str,
    path_value: str,
    ...
    if not required.issubset(records.columns):
        return pd.DataFrame()
    output = records.copy()
    output["player_1_canonical_id"] = output.get("batter_1", "")
    output["player_2_canonical_id"] = output.get("batter_2", "")
    output["player_1_name"] = output["batter_1_public_name"]
    output["player_2_name"] = output["batter_2_public_name"]
    output["runs"] = pd.to_numeric(output["partnership_runs"], errors="coerce")
    output["balls"] = pd.to_numeric(output.get("balls_faced"), errors="coerce")
    output["wicket_number"] = pd.to_numeric(output["wicket_number"], errors="coerce")
    output["display_grade_name"] = output.get("grade", "")
    output["raw_grade_name"] = output.get("grade", "")
    output["team_name"] = output.get("batting_team", "")
    output["source_classification"] = "ball_by_ball_calculated"
    private_1 = output["player_1_name"].eq("Private player")
    private_2 = output["player_2_name"].eq("Private player")
    output.loc[private_1, "player_1_canonical_id"] = ""
    output.loc[private_2, "player_2_canonical_id"] = ""
    return output.sort_values("wicket_number").reset_index(drop=True)
...
def render_partnership_records(team_group_slug: str | None = None) -> None:
    ...
    render_partnership_record_panels(
        records,
        empty_title="Record Partnerships",
        empty_message="No confirmed partnership records are available.",
    )


def render_partnership_record_panels(
    ...
    *,
    ...
    for column, start, end in [(columns[0], 1, 5), (columns[1], 6, 10)]:
        rows = records[
            pd.to_numeric(records["wicket_number"], errors="coerce").between(
                start, end, inclusive="both"
            )
        ]
        rows_by_wicket = {
            int(float(row["wicket_number"])): row
            for _, row in rows.iterrows()
            if pd.notna(pd.to_numeric(row.get("wicket_number"), errors="coerce"))
        }
        row_html = "".join(
            partnership_record_row_html(rows_by_wicket[wicket])
            if wicket in rows_by_wicket
            else partnership_record_empty_row_html(wicket)
            for wicket in range(start, end + 1)
        )
        with column:
            st.markdown(
...
def partnership_record_empty_row_html(wicket_number: int) -> str:
    return (
        '<div class="performance-row partnership-record-empty">'
        f'<span class="progress-rank">{wicket_number}</span>'
        '<div class="performance-player">'
        '<strong>No verified record available</strong>'
        '<span>Available ball-by-ball coverage contains no eligible partnership for this wicket.</span>'
        '</div>'
...
def partnership_record_row_html(row: pd.Series) -> str:
    ...
    player_1_link = (
        ...
        else player_profile_link_html(row.get("player_1_canonical_id"), player_1)
    )
    player_2_link = (
        html.escape(player_2)
        if player_2 == "Private player"
        else player_profile_link_html(row.get("player_2_canonical_id"), player_2)
    )
...
def clean_opponent_label(value: object, fallback: str = "Unknown opposition") -> str:
    ...
    normalized = normalize_opponent_club_name(text, fallback=fallback)
    return canonical_app_opponent_label(normalized, fallback)


FVCC_OPPONENT_DISPLAY_MERGES = {
...
def canonical_app_opponent_label(value: object, fallback: str = "Unknown opposition") -> str:
    text = safe_record_text(value, fallback)
    if get_active_club_id() == "fvcc":
        return FVCC_OPPONENT_DISPLAY_MERGES.get(text.casefold(), text)
    return text


def canonical_grdcc_profile_opponent(value: object) -> str:
    text = safe_record_text(value, "Unknown opponent")
    key = re.sub(r"\s+", " ", text.casefold().replace("–", "-").replace("—", "-")).strip()
    explicit_merges = {
        "balmain south sydney 1st grade cricket club": "Balmain South Sydney Cricket Club",
        "balmain south sydney 2nd grade cricket club": "Balmain South Sydney Cricket Club",
        "balmain south sydney cricket club": "Balmain South Sydney Cricket Club",
        "burwood briars 1st grade cricket club": "Burwood Briars Cricket Club",
        "burwood briars 2nd grade cricket club": "Burwood Briars Cricket Club",
        "burwood briars cricket club": "Burwood Briars Cricket Club",
        "epping bulls 1st grade cricket club": "Epping Bulls Cricket Club",
        "epping bulls 1st grade l/o cricket club": "Epping Bulls Cricket Club",
        "epping bulls cricket club": "Epping Bulls Cricket Club",
        "lane cove 1st grade cricket club": "Lane Cove Cricket Club",
        "lane cove 2nd grade cricket club": "Lane Cove Cricket Club",
        "lane cove cricket club": "Lane Cove Cricket Club",
        "lindfield 1st grade cricket club": "Lindfield Cricket Club",
        "lindfield 2nd grade cricket club": "Lindfield Cricket Club",
        "lindfield cricket club": "Lindfield Cricket Club",
        "mt pritchard southern districts 1st grade cricket club": "Mt Pritchard-Southern Districts Cricket Club",
        "mt pritchard southern districts 2nd grade cricket club": "Mt Pritchard-Southern Districts Cricket Club",
        "mt pritchard-southern districts cricket club": "Mt Pritchard-Southern Districts Cricket Club",
        "north west sydney 1st grade cricket club": "North West Sydney Cricket Club",
        "north west sydney 2nd grade cricket club": "North West Sydney Cricket Club",
        "north west sydney cricket club": "North West Sydney Cricket Club",
        "pennant hills 1st grade cricket club": "Pennant Hills Cricket Club",
        "pennant hills 2nd grade cricket club": "Pennant Hills Cricket Club",
        "pennant hills cricket club": "Pennant Hills Cricket Club",
        "roseville 1st grade cricket club": "Roseville Cricket Club",
        "roseville cricket club": "Roseville Cricket Club",
        "strathfield 1st grade cricket club": "Strathfield Cricket Club",
        "strathfield 2nd grade cricket club": "Strathfield Cricket Club",
        "strathfield cricket club": "Strathfield Cricket Club",
    }
    canonical = explicit_merges.get(key)
    if canonical:
        return canonical
    if re.fullmatch(r"Warringah [1-5](?:st|nd|rd|th) Grade(?: L/O)? Cricket Club", text, flags=re.IGNORECASE):
        return "Warringah Cricket Club"
    if re.fullmatch(r"Auburn [1-5](?:st|nd|rd|th) Grade(?: L/O)? Cricket Club", text, flags=re.IGNORECASE):
        return "Auburn Cricket Club"
    grade_team_pattern = re.compile(
        r"^(?P<club>.+?)\s+(?:(?:1st|2nd|3rd|4th|5th|first|second|third|fourth|fifth)\s+grade)(?:\s+(?:l/o|limited\s+overs))?\s+cricket\s+club$",
        flags=re.IGNORECASE,
    )
    match = grade_team_pattern.match(text)
    if match:
        club = re.sub(r"\s+", " ", match.group("club")).strip()
        if club:
            return f"{club} Cricket Club"
    return text
...
def record_card_html(card: dict[str, object]) -> str:
    if card.get("leader_details"):
        return leader_highlight_card_html(card)
    ...
    card_class = str(card.get("card_class") or "record-card")
    value = str(card.get("value", ""))
    value_html = value if card.get("value_html") else html.escape(value)
    return (
        ...
        f'<div class="record-value">{value_html}</div>'
        f"{meta}"
        f"{scorecard_meta}"
        "</div>"
    )
...
def milestone_club_card_html(
    ...
    return (
        ...
        "</div>"
        '<div class="milestone-member-list" data-visible-rows="5" data-scroll-enabled="true">'
        f'{"".join(member_rows)}</div>'
        "</article>"
    )
...
def add_club_not_outs_for_display(
    all_time: pd.DataFrame,
    batting: pd.DataFrame,
    club_id: str | None = None,
) -> pd.DataFrame:
    active_club = club_id or get_active_club_id()
    if active_club == "glen-waverley-hawks":
        return add_gwhcc_not_outs_for_display(all_time, batting)
    if active_club == "fvcc":
        return add_complete_not_outs_for_display(all_time, batting)
...
def hof_sortable_table_html(table: pd.DataFrame, key_prefix: str) -> str:
    ...
    {'<div class="hof-matches-footnote">* For players where match counts were not captured, innings are shown as a match-count proxy.</div>' if matches_proxy_used else ''}
    <script>
      (() => {{
...
def build_player_profile_v2_context(profile_view: dict[str, pd.DataFrame]) -> dict[str, object]:
    ...
    _, premierships = load_premiership_records(get_active_club_id(), premiership_records_signature())
...
def load_player_profile_index(
    ...
    for category in ["batting", "bowling", "fielding"]:
        frame = read_processed_table(f"all_seasons_{category}")
        if not frame.empty:
            frames.append(apply_player_identity_mapping(frame, aliases, club_id=club_id))
    if not frames:
...
def build_player_season_table(
    ...
    for season in seasons:
        ...
        not_outs = profile_not_outs_coverage(bat, get_active_club_id())
        row = {
            "Season": season,
            ...
            "Not Outs": not_outs.value if not_outs.status == NOT_OUTS_COMPLETE else pd.NA,
            "not_outs_coverage_status": not_outs.status,
...
def build_player_career_totals(
    ...
    if season_table.empty:
        return pd.DataFrame()
    not_outs = profile_not_outs_coverage(batting, get_active_club_id())
    totals = {
...
def profile_badge_class(label: str) -> str:
    ...
    if base.casefold().startswith("premiership"):
...
def player_premiership_profile_badges(career: pd.Series) -> list[str]:
    ...
    if winner_count > 0:
        badges.append(counted_profile_badge("Premiership Winning Player", winner_count))
    return badges


@st.cache_data(show_spinner=False, persist="disk")
def latest_profile_activity_seasons(club_id: str, local_version: float) -> tuple[str, ...]:
    _ = (club_id, local_version)
    seasons: set[str] = set()
    for category in ["batting", "bowling", "fielding"]:
        frame = read_processed_table(f"all_seasons_{category}")
...
def profile_badge_sort_key(candidate: dict[str, object]) -> tuple[int, int]:
    ...
    if base in {"Premiership Winner", "Premiership Winning Player"}:
...
def player_highlight_cards(profile_view: dict[str, pd.DataFrame]) -> list[dict[str, object]]:
    career = profile_view["career"].iloc[0]
        ...
        if metric in season_table:
            ...
            if not output.empty:
                ...
                cards.append(
                    {
                        "title": title,
                        "player": str(row["Season"]),
                        "value": f"{int(row[metric]):,} {suffix}",
                        "meta": clean_highlight_context_label(row.get("Teams/Grades", "")),
                        "link_type": "season",
                    }
                )
    fifties = int(numeric_value(career, "50s"))
    hundreds = int(numeric_value(career, "100s"))
    if fifties or hundreds:
        if fifties and hundreds:
            title = "Total 50s/100s"
            value = f"{fifties:,} {plural_word(fifties, 'fifty', 'fifties')} /<br>{hundreds:,} {plural_word(hundreds, 'hundred', 'hundreds')}"
            value_html = True
        elif fifties:
            title = "Total 50s"
            value = f"{fifties:,} {plural_word(fifties, 'fifty', 'fifties')}"
            value_html = False
        else:
            title = "Total 100s"
            value = f"{hundreds:,} {plural_word(hundreds, 'hundred', 'hundreds')}"
            value_html = False
        cards.append(
            {
                "title": title,
                ...
                "meta": "",
                "card_class": "record-card record-card-compact-value",
            }
        )
    five_wicket_hauls = int(numeric_value(career, "5WI"))
    if five_wicket_hauls > 0:
        cards.append({"title": "Total 5-Wicket Hauls", "player": str(career["Player"]), "value": f"{five_wicket_hauls:,} five-wicket hauls", "meta": ""})
    leader_details = player_leader_details(profile_view)
    for title, key, suffix in [
        ...
        if value > 0:
            ...
            if "grade" in key:
                label = "grade-leading season" if value == 1 else "grade-leading seasons"
            else:
                label = "club-leading season" if value == 1 else "club-leading seasons"
            cards.append(
                {
                    "title": title,
                    "player": str(career["Player"]),
                    ...
                    "leader_by_grade": "grade" in key,
                }
            )
    return cards[:10]


def leader_highlight_card_html(card: dict[str, object]) -> str:
    details = [str(value) for value in card.get("leader_details", []) if str(value).strip()]
    by_grade = bool(card.get("leader_by_grade"))
    details = sorted(
        details,
        key=lambda value: profile_season_sort_key(value.partition(" · ")[0]),
        reverse=True,
    )
    visible = details[:2]
    detail_rows = []
    for detail in visible:
        ...
        grade_label = clean_highlight_context_label(grade)
        title = f"{grade_label} - {season.strip()}" if by_grade and separator and grade_label else season.strip()
        chip_grade = compact_leader_highlight_label(grade_label)
        season_label = season.strip()
        chip_content = (
            '<span class="leader-highlight-chip-grade">'
            f'{html.escape(chip_grade)}'
            '</span>'
            '<span class="leader-highlight-chip-season">'
            f'{html.escape(season_label)}'
            '</span>'
            if by_grade and separator and chip_grade
    ...
    if remaining > 0:
        detail_rows.append(f'<span class="leader-highlight-more">+ {remaining} more</span>')
    return (
        '<div class="record-card leader-highlight-card">'
        f'<div class="record-label">{html.escape(str(card.get("title", "")))}</div>'
        f'<div class="leader-highlight-count">{html.escape(str(card.get("value", "")))}</div>'
        f'<div class="leader-highlight-details">{"".join(detail_rows)}</div>'
        '</div>'
    )
...
def clean_highlight_context_label(value: object) -> str:
    text = safe_record_text(value, "")
    if not text:
        return ""
    parts = [
        ...
        if part.strip() and part.strip().casefold() != "historical club summary"
    ]
    if not parts and text.casefold() == "historical club summary":
        return ""
    cleaned = " - ".join(parts) if parts else text
    if get_active_club_id() == "georges-river-district":
        return clean_profile_grade_label(cleaned)
    return cleaned


def compact_leader_highlight_label(value: object) -> str:
    label = safe_record_text(value, "")
    compact = {
        "First Grade The RB Clark Cup": "First Grade",
        "Second Grade The SJ Mayne Trophy": "Second Grade",
        "Third Grade The JB Hollander Cup": "Third Grade",
        "Fourth Grade The Harry Culbert Trophy": "Fourth Grade",
        "Fifth Grade The Tim Creer Cup": "Fifth Grade",
...
def player_leader_details(profile_view: dict[str, pd.DataFrame]) -> dict[str, list[str]]:
    ...
    if not player_id:
        return {}
    return cached_player_leader_details(
        player_id,
        get_active_club_id(),
        metadata_mtime(),
        player_aliases_mtime(),
...
@st.cache_data(show_spinner=False, persist="disk")
def cached_player_leader_details(
    player_id: str,
    club_id: str,
    local_version: float,
    identity_version: float,
) -> dict[str, list[str]]:
    historical_data = load_hall_of_fame_data(local_version, identity_version, club_id=club_id)
    if historical_data is None:
        return {}
...
def profile_best_position_label(rows: pd.DataFrame) -> str:
    if rows.empty:
        return ""
    minimum_innings = player_best_position_min_innings(pd.to_numeric(rows.get("innings"), errors="coerce").fillna(0).sum())
    eligible = rows[pd.to_numeric(rows.get("innings"), errors="coerce").fillna(0) >= minimum_innings].copy()
...
def player_best_position_min_innings(total_innings: object) -> int:
    numeric = pd.to_numeric(total_innings, errors="coerce")
    if pd.isna(numeric) or float(numeric) <= 0:
        return 1
    return max(1, math.floor(float(numeric) * 0.11))
...
def profile_best_phase_label(rows: pd.DataFrame) -> str:
    ...
    output["_avg_sort"] = pd.to_numeric(output.get("avg"), errors="coerce")
    output = output[
        (output["_balls_sort"] >= 60)
        & output["_avg_sort"].notna()
        & output["_avg_sort"].map(lambda value: math.isfinite(float(value)) if pd.notna(value) else False)
    ].copy()
    if output.empty:
        return ""
    output["_phase_order_sort"] = pd.to_numeric(output.get("phase_order"), errors="coerce").fillna(99)
    output = output.sort_values(["_avg_sort", "_phase_order_sort", "_balls_sort"], ascending=[True, True, False])
...
def render_player_peer_comparison(profile_view: dict[str, pd.DataFrame]) -> None:
    ...
    club_id = get_active_club_id()
    comparison = get_player_peer_comparison(player_id, seasons, peer_scope, metadata_mtime(), player_aliases_mtime(), club_id)
    ...
    columns = st.columns(2)
    peer_accent = active_chart_primary_colour()
    with columns[0]:
        render_peer_comparison_card("Batting", comparison.get("batting", []), peer_accent)
    with columns[1]:
...
def peer_metric_note(label: object) -> str:
    notes = {
        ...
        "Duck %": "% of dismissals that were ducks",
        "Maiden %": "% of overs bowled that were maidens",
        "Extras %": "% of balls bowled that were wides or no-balls",
    }
...
def render_player_performance_breakdown(profile_view: dict[str, pd.DataFrame]) -> None:
    ...
    with st.container(key="player_profile_performance_breakdown"):
        ...
        if source.empty:
            render_profile_breakdown_fallback_table(profile_view, selected, selected_discipline)
            return
        source = normalize_player_profile_breakdown_groups(source)
...
def render_profile_breakdown_fallback_table(
    profile_view: dict[str, pd.DataFrame],
    selected_breakdown: str,
    selected_discipline: str,
) -> None:
    label = profile_breakdown_label(selected_breakdown)
    if selected_breakdown == "season":
        ...
        if not rows.empty:
            ...
            render_profile_season_stat_table(rows, columns, ["Catches", "Stumpings", "Run Outs", "Dismissals"])
            return
    if selected_breakdown == "grade":
        ...
        if not rows.empty:
            ...
            if selected_discipline == "Bowling":
                ...
                render_profile_group_stat_table(
                    table,
                    ["Grade", "Matches", "Overs", "Wickets", "Bowl Avg", "Bowl SR", "Eco", "BBI", "3WI", "5WI", "Extras %"],
                    ["Balls Bowled", "Runs Against", "Wickets", "Wides", "No Balls", "3WI", "5WI"],
                    "Grade",
                )
                return
...
def profile_batting_table_columns(label_column: str) -> list[str]:
    ...
    if get_active_club_id() == "glen-waverley-hawks":
        columns.insert(2, "NO")
    elif get_active_club_id() in {"fvcc", "georges-river-district"}:
...
def normalize_player_profile_breakdown_groups(source: pd.DataFrame) -> pd.DataFrame:
    if source.empty or "dimension" not in source or "breakdown_label" not in source:
        return source.copy()
    output = source.copy()
    opponent_mask = output["dimension"].astype(str).eq("Opponent")
    if opponent_mask.any():
        opponent_normalizer = (
            canonical_grdcc_profile_opponent
            if get_active_club_id() == "georges-river-district"
            else canonical_app_opponent_label
        )
        output.loc[opponent_mask, "breakdown_label"] = output.loc[opponent_mask, "breakdown_label"].map(
            opponent_normalizer
        )
    if get_active_club_id() == "georges-river-district":
        grade_mask = output["dimension"].astype(str).eq("Grade")
        output.loc[grade_mask, "breakdown_label"] = output.loc[grade_mask, "breakdown_label"].map(
            clean_profile_grade_label
        )
    return output


def combine_profile_breakdown_rows(rows: pd.DataFrame, discipline: str) -> pd.DataFrame:
    if rows.empty or "breakdown_label" not in rows or not rows["breakdown_label"].duplicated().any():
        return rows
    output = rows.copy()
    common = {"dimension": "first", "dimension_order": "min", "discipline": "first"}
    if discipline == "Batting":
        sums = ["innings", "runs", "outs", "thirties", "fifties", "hundreds", "ducks", "fours", "sixes", "bbb_runs", "bbb_balls_faced", "bbb_dismissals", "bbb_batting_innings", "bbb_matches"]
        aggregations = {column: "sum" for column in sums if column in output}
        if "high_score" in output:
            aggregations["high_score"] = best_high_score_from_display_values
    elif discipline == "Bowling":
        sums = ["matches", "balls_bowled", "runs_against", "wickets", "three_wicket_innings", "five_wicket_innings", "wides", "no_balls"]
        aggregations = {column: "sum" for column in sums if column in output}
        if "bbi" in output:
            aggregations["bbi"] = best_bbi_from_display_values
    else:
        aggregations = {
            column: "sum"
            for column in ["catches", "stumpings", "run_outs", "dismissals"]
            if column in output
        }
    aggregations.update({column: method for column, method in common.items() if column in output})
    grouped = output.groupby("breakdown_label", dropna=False, as_index=False).agg(aggregations)
    if discipline == "Batting":
        grouped["bat_avg"] = grouped.apply(lambda row: divide_or_none(row.get("runs", 0), row.get("outs", 0)), axis=1)
        grouped["strike_rate"] = grouped.apply(
            lambda row: divide_or_none(row.get("bbb_runs", 0) * 100, row.get("bbb_balls_faced", 0)), axis=1
        )
    elif discipline == "Bowling":
        grouped["bowl_avg"] = grouped.apply(lambda row: divide_or_none(row.get("runs_against", 0), row.get("wickets", 0)), axis=1)
        grouped["bowl_sr"] = grouped.apply(lambda row: divide_or_none(row.get("balls_bowled", 0), row.get("wickets", 0)), axis=1)
        grouped["eco"] = grouped.apply(lambda row: divide_or_none(row.get("runs_against", 0) * 6, row.get("balls_bowled", 0)), axis=1)
        grouped["extras_pct"] = grouped.apply(
            lambda row: calculate_extras_pct(row.get("no_balls"), row.get("wides"), row.get("balls_bowled")), axis=1
        )
    return grouped
...
def render_profile_performance_table(
    ...
    filtered = combine_profile_breakdown_rows(filtered, discipline)
    if filtered.empty:
    ...
    if discipline == "Batting":
        ...
        if get_active_club_id() in {"fvcc", "glen-waverley-hawks"} and {"Inn", "Outs"}.issubset(table.columns):
            table["Not Outs"] = (
...
def profile_performance_table_html(
    ...
    link_colour = active_link_colour()
    link_hover_colour = active_link_hover_colour()
    ...
    <style>
      html, body {{
        background: transparent;
        --profile-table-link: {html.escape(link_colour)};
        --profile-table-link-hover: {html.escape(link_hover_colour)};
        color-scheme: light;
        font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif;
        margin: 0;
        padding: 0;
      ...
      .profile-performance-table .profile-label-link:hover {{
        color: var(--profile-table-link-hover);
        text-decoration: underline;
      }}
...
def profile_performance_display_value(column: str, value: object, label_column: str, row: pd.Series | None = None) -> str:
    if str(column) == str(label_column):
        return profile_performance_label_cell(value, label_column)
...
def profile_performance_label_cell(value: object, label_column: str = "") -> str:
    if pd.isna(value) or str(value).strip() == "":
        return ""
    text = str(value).strip()
    label = link_display_label(text)
    if label_column == "Season":
        text = season_overview_url(label or text)
    elif label_column == "Captain":
        player_id = profile_player_id_for_name(label or text)
        text = player_profile_url(player_id, label or text) if player_id else text
    if text.startswith("?"):
        return (
            f'<a class="profile-label-link" href="{html.escape(text, quote=True)}" '
            f'data-profile-performance-link="1" target="_blank" rel="noopener noreferrer" '
...
def profile_player_id_for_name(player_name: object) -> str:
    name_key = player_name_match_key(player_name)
    if not name_key:
        return ""
    index = load_player_profile_index(
    ...
    return str(matched.iloc[0]["id"]).strip() if not matched.empty else ""
...
def sort_profile_performance_table(table: pd.DataFrame, label_column: str, discipline: str) -> pd.DataFrame:
    ...
    if label_column == "Grade":
        sort_function = grdcc_career_breakdown_grade_sort_key if get_active_club_id() == "georges-river-district" else grade_sort_key
        output["_sort_key"] = output[label_column].map(sort_function)
...
GRDCC_CAREER_BREAKDOWN_GRADE_ORDER = [
    "First Grade",
    "Second Grade",
    "Third Grade",
    "Fourth Grade",
...
def grdcc_career_breakdown_grade_sort_key(value: object) -> tuple[int, str]:
    label = clean_profile_grade_label(value)
    normalized = re.sub(r"[^a-z0-9]+", " ", str(label or "").casefold()).strip()
    for index, grade in enumerate(GRDCC_CAREER_BREAKDOWN_GRADE_ORDER):
        grade_key = re.sub(r"[^a-z0-9]+", " ", grade.casefold()).strip()
        if normalized == grade_key or normalized.startswith(f"{grade_key} "):
            return (index, normalized)
    return (len(GRDCC_CAREER_BREAKDOWN_GRADE_ORDER), normalized)
...
def render_player_breakdown(career: pd.Series) -> None:
    ...
    render_grdcc_not_outs_career_note(career)
...
def player_profile_batting_career_metrics(career: pd.Series, matches_text: str) -> list[tuple[str, str]]:
    ...
    elif get_active_club_id() == "fvcc":
        not_outs = pd.to_numeric(career.get("Not Outs"), errors="coerce")
        if pd.isna(not_outs):
            innings = pd.to_numeric(career.get("Innings"), errors="coerce")
            outs = pd.to_numeric(career.get("Outs"), errors="coerce")
            not_outs = max(float(innings) - float(outs), 0) if pd.notna(innings) and pd.notna(outs) else None
        metrics.insert(1, ("Not Outs", format_int(not_outs)))
    elif get_active_club_id() == "georges-river-district":
        not_outs = pd.to_numeric(career.get("Not Outs"), errors="coerce")
        metrics.insert(1, ("Not Outs", "N/A" if pd.isna(not_outs) else format_int(not_outs)))
    return metrics
...
def render_grdcc_not_outs_career_note(career: pd.Series) -> None:
    if get_active_club_id() != "georges-river-district":
        return
    if str(career.get("not_outs_coverage_status", "")).casefold() in {NOT_OUTS_PARTIAL, "unavailable"}:
        st.caption("Not Outs reflects available digital scorecard coverage; historical records may be incomplete.")


def render_grdcc_not_outs_table_note() -> None:
    if get_active_club_id() == "georges-river-district":
        st.caption("Not Outs reflects available digital scorecard coverage; historical records may be incomplete.")
...
def clean_profile_grade_from_row(row: pd.Series) -> str:
    ...
    return canonical_profile_grade_label(grade or team or "Unknown grade")
...
def clean_profile_grade_label(value: object) -> str:
    return canonical_profile_grade_label(clean_grade_name(value).replace("Designated One Day Comp.", "DODC"))


def canonical_profile_grade_label(value: object) -> str:
    text = safe_record_text(value, "")
    if get_active_club_id() == "georges-river-district":
        return clean_grade_name(text)
    return text
...
def profile_record_meta_html(row: pd.Series) -> str:
    ...
    if pd.notna(season):
        parts.append(season_overview_link_html(season))
    grade = row.get("canonical_grade_label") or canonical_grade_label(row.get("team_name", ""), row.get("grade_name", ""))
    grade = clean_highlight_context_label(grade)
    if grade and grade != "—":
        parts.append(html.escape(str(grade)))
    return " - ".join(parts)
...
def historical_matches_display_text(
    ...
    *,
    ...
    if is_historical_season and (pd.isna(matches) or float(matches) <= 0) and (has_real_stats or has_bowling_overs):
        if has_bowling_overs:
            overs_number = pd.to_numeric(pd.Series([overs_value]), errors="coerce").iloc[0]
            if pd.notna(overs_number) and float(overs_number) > 0:
                display_value = int(round(float(overs_number)))
                return f"{display_value:,}*", display_value, True
        return "", None, False
    if pd.isna(matches):
        return "", None, False
...
def previous_season_for(current_season: dict[str, object]) -> dict[str, object] | None:
    seasons = load_local_playcricket_seasons(get_active_club_id(), metadata_mtime())
    if not seasons:
        return None
    current_id = str(current_season.get("id", ""))
...
def render_full_stats_section(dashboard_data: dict[str, object]) -> None:
    ...
    with st.container(key="full_stats_card"):
        ...
        with batting_tab:
            ...
            render_grdcc_not_outs_table_note()
        with bowling_tab:
            render_full_stats_table(
...
def season_overview_detail_table_html(table: pd.DataFrame, category: str, table_id: str) -> str:
    ...
    header_html = "".join(
        (
            ...
            f'<button type="button">{html.escape(season_detail_header_label(column))}'
    ...
    <style>
      ...
      .season-detail-table col.season-col-dis {{ width: 44px; }}
      ...
      .season-detail-table col.season-col-extras {{ width: 58px; }}
      ...
      @media (max-width: 760px) {{
        ...
        .season-detail-table col.season-col-5wi,
        .season-detail-table col.season-col-no-balls,
        .season-detail-table col.season-col-wides {{ width: 38px; }}
        ...
        .season-detail-table col.season-col-extras {{ width: 54px; }}
        .season-detail-table th,
        .season-detail-table td {{
          padding: 7px 6px;
        }}
...
def season_detail_header_label(column: object) -> str:
    return {
        "No Balls": "NB",
        "Wides": "Wd",
...
def get_batting_display_df(df: pd.DataFrame) -> pd.DataFrame:
    df = mask_historical_not_outs(df, get_active_club_id())
    if get_active_club_id() in {"fvcc", "georges-river-district"} and "battingNotOuts" in df:
        name_column = "canonical_player_name" if "canonical_player_name" in df else "player_name"
        if name_column in df:
            df.loc[df[name_column].map(is_private_or_anonymised_player), "battingNotOuts"] = pd.NA
    source_columns = [
        "player_name",
        "team_name",
        "matches",
    ...
    if get_active_club_id() in {"fvcc", "georges-river-district", "glen-waverley-hawks"}:
