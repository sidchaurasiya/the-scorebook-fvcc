...
def render_page() -> None:
    ...
    elif selected_page == "Near Milestones":
...
def render_hall_of_fame_page() -> None:
    ...
    st.markdown(
        """
        <div class="page-kicker">All available seasons</div>
        <div class="club-label">Fiji Victorian Cricket Club</div>
        <h1 class="page-title">Hall of Fame</h1>
        <div class="page-subtitle">The players who shaped the club’s history.</div>
        <div class="context-line hof-context">
            <span>Based on all available seasons in local backup.</span>
...
def render_approaching_milestones_page() -> None:
    ...
    st.markdown(
        """
        <div class="page-kicker">All-time milestone watch</div>
        <div class="club-label">Fiji Victorian Cricket Club</div>
        <h1 class="page-title">Near Milestones</h1>
        <div class="page-subtitle">Players closing in on major FVCC career landmarks.</div>
        <div class="context-line hof-context">
            <span>Milestones are calculated from all available historical FVCC data.</span>
            <span class="source-note">Source: PlayCricket Australia</span>
        </div>
        """,
        unsafe_allow_html=True,
    )
    render_milestone_kpis(watchlist)
    render_career_milestone_cards(watchlist)
    render_milestone_watchlist_table(watchlist)
...
def build_player_identity_frame(frames: list[pd.DataFrame]) -> pd.DataFrame:
    ...
    return (
        ...
        .agg({"player_name": "first", "teams_grades": join_unique_csv, "seasons_played": "max"})
...
def render_hall_of_fame_kpis(data: dict[str, object]) -> None:
    cards = [
        ("Total Seasons Analysed", f"{int(data['total_seasons']):,}", "", "seasons", "▦", "purple"),
        ("Total Players", f"{int(data['total_players']):,}", "", "players", "♙", "blue"),
        ("Total Matches Recorded", f"{int(data['total_matches']):,}", "", "matches", "▣", "blue"),
        ("Total Runs / Wickets", f"{int(data['total_runs']):,} / {int(data['total_wickets']):,}", "", "records", "♕", "purple"),
    ]
...
def render_hall_of_fame_leaders(all_time: pd.DataFrame) -> None:
    ...
    leader_specs = [
        ...
        ("Most Stumpings", "Stumpings", "stumpings", "fielding"),
        ("Most Run Outs", "Run Outs", "run outs", "fielding"),
        ("Most Matches", "Matches", "matches", "fielding"),
    ]
    for index in range(0, len(leader_specs), 3):
        columns = st.columns(3)
        for column, (title, metric, suffix, mode) in zip(columns, leader_specs[index : index + 3]):
            with column:
...
def build_record_holder_cards(data: dict[str, object]) -> list[dict[str, str]]:
    ...
    if not batting_raw.empty and "battingHighScore" in batting_raw:
        output = batting_raw.copy()
        output["battingHighScore"] = pd.to_numeric(output["battingHighScore"], errors="coerce")
        output = output[output["battingHighScore"].notna()].sort_values("battingHighScore", ascending=False)
        if not output.empty:
            row = output.iloc[0]
            cards.append(
                {
    ...
    if not bowling_raw.empty and "bowlingBestInnings" in bowling_raw:
        bbi = sort_bowling_by_bbi(bowling_raw.dropna(subset=["bowlingBestInnings"]))
        if not bbi.empty:
            row = bbi.iloc[0]
            cards.append(
                {
    ...
    for title, metric, suffix in [
        ...
        ("Most 5-Wicket Hauls", "5WI", "five-wicket hauls"),
        ("Most Ducks", "0s", "ducks"),
    ]:
...
def render_milestone_club(all_time: pd.DataFrame) -> None:
    groups = [
        ("1000+ Runs", "Runs", 1000, "runs"),
        ("500+ Runs", "Runs", 500, "runs"),
        ("100+ Wickets", "Wickets", 100, "wickets"),
        ("50+ Wickets", "Wickets", 50, "wickets"),
        ("100+ Matches", "Matches", 100, "matches"),
        ("50+ Catches", "Catches", 50, "catches"),
    ]
    rendered = []
    for title, metric, threshold, suffix in groups:
        if metric not in all_time:
            continue
        players = all_time.copy()
        players[metric] = pd.to_numeric(players[metric], errors="coerce").fillna(0)
        players = players[players[metric] >= threshold].sort_values(metric, ascending=False)
        if players.empty:
            continue
        chips = "".join(
            f'<span class="milestone-chip">{html.escape(str(row["Player"]))} · {int(row[metric]):,} {html.escape(suffix)}</span>'
            for _, row in players.head(18).iterrows()
        )
        rendered.append(f'<div class="milestone-group"><h4>{html.escape(title)}</h4><div>{chips}</div></div>')
    if not rendered:
        return
...
def render_detailed_all_time_records(all_time: pd.DataFrame) -> None:
    ...
    table = format_all_time_table(all_time)
...
def render_career_milestone_cards(watchlist: pd.DataFrame) -> None:
    ...
    career_categories = ["Matches", "Runs", "Wickets", "Catches", "Dismissals"]
    for index in range(0, len(career_categories), 2):
        columns = st.columns(2)
        for column, category in zip(columns, career_categories[index : index + 2]):
            with column:
                render_milestone_category_card(watchlist, category)

    special = watchlist[watchlist["Group"] == "Special Achievements"] if not watchlist.empty else pd.DataFrame()
    if not special.empty:
        render_section_heading("Special Achievements")
        columns = st.columns(3)
        for column, category in zip(columns, ["Half-centuries", "Centuries", "Five-wicket hauls"]):
            with column:
                render_milestone_category_card(watchlist, category)
